...
class GRUEmbedding(nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        self.Wx0 = nn.Embedding(dim_in, dim_out * 3)
...
class GRU4RecModel(nn.Module):
    def __init__(self, n_items, layers=[100], dropout_p_embed=0.0, dropout_p_hidden=0.0, embedding=0, constrained_embedding=True):
        ...
        if constrained_embedding:
            ...
            self.E = nn.Embedding(n_items, embedding)
            n_input = embedding
        else:
        ...
        self.Wy = nn.Embedding(n_items, layers[-1])
        self.By = nn.Embedding(n_items, 1)
        self.reset_parameters()
    @torch.no_grad()
    def reset_parameters(self):
        if self.embedding:
...
class GRU4RecWrapper(nn.Module):
    """Wrapper that adapts the official GRU4RecModel to the training pipeline interface."""
    ...
    def __init__(self, user_num, item_num, args):
        ...
        layers = list(args.model_args.get('layers', [64]))
        dropout_p_embed = args.model_args.get('dropout_p_embed', args.model_args.dropout_rate)
        dropout_p_hidden = args.model_args.get('dropout_p_hidden', args.model_args.dropout_rate)
        embedding = args.model_args.get('embedding', 0)
        constrained_embedding = args.model_args.get('constrained_embedding', True)

        # +1 because item IDs are 1-indexed, index 0 is padding
        ...
        # Expose item_emb for L2 reg compatibility in main.py
        self.item_emb = self.model.Wy

    def log2feats(self, log_seqs):
        """Process padded sequence through GRU cells one timestep at a time."""
        log_seqs = torch.tensor(log_seqs).to(self.dev).long()
        B, T = log_seqs.shape
        layers = self.model.layers

        H = [torch.zeros(B, layers[i], device=self.dev) for i in range(len(layers))]
        ...
        for t in range(T):
            ...
            Xh = E
            for i in range(self.model.start, len(layers)):
                Xh = self.model.G[i](Xh, H[i])
                if self.training:
                    Xh = self.model.D[i](Xh)
                H[i] = Xh
    ...
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs):
        ...
        pos_seqs = torch.tensor(pos_seqs).to(self.dev).long()
        neg_seqs = torch.tensor(neg_seqs).to(self.dev).long()

        pos_embs = self.model.Wy(pos_seqs)
        neg_embs = self.model.Wy(neg_seqs)
        pos_bias = self.model.By(pos_seqs).squeeze(-1)
        neg_bias = self.model.By(neg_seqs).squeeze(-1)

        pos_logits = (log_feats * pos_embs).sum(dim=-1) + pos_bias
        neg_logits = (log_feats * neg_embs).sum(dim=-1) + neg_bias
    ...
    def predict(self, user_ids, log_seqs, item_indices):
        ...
        item_indices = torch.tensor(item_indices).to(self.dev).long()
        item_embs = self.model.Wy(item_indices)
        item_bias = self.model.By(item_indices).squeeze(-1)

        logits = item_embs.matmul(final_feat.unsqueeze(-1)).squeeze(-1) + item_bias
        return logits